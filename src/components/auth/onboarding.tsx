...
import { ArrowRight, Sparkles, ShieldCheck, ChevronLeft } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import { cn } from "@/lib/utils";

export const Onboarding = () => {
    const { setUserId, onboardGuest, signInWithGoogle } = useProductivity();
                    ...
                    >
                        ...
                        <div className="flex flex-col md:flex-row items-center justify-center gap-4">
                            ...
                            <button
                                ...
                                className="flex items-center gap-3 bg-white text-slate-900 px-8 py-5 text-base font-black rounded-[2rem] hover:scale-105 active:scale-95 transition-all shadow-2xl shadow-white/10"
                            >
                            ...
                            <button
                                ...
                                disabled={isLoading}
                                className="bg-white/10 border border-white/20 text-white px-8 py-5 text-base font-black rounded-[2rem] hover:bg-white/20 active:scale-95 transition-all flex items-center justify-center shadow-lg disabled:opacity-50"
                            >
                                {isLoading ? "Generating..." : "New Guest Workspace"}
                                {!isLoading && <ArrowRight className="ml-2 w-5 h-5" />}
                            </button>
                            <button
                                onClick={handleNext}
                                className="bg-white/5 border border-white/10 text-white/60 px-8 py-5 text-base font-bold rounded-[2rem] hover:bg-white/10 transition-all active:scale-95"
                            >
                                Recover Session
                            </button>
                        </div>
